...
def data_root() -> Path:
    """Główny katalog danych EDNOR.

    Opcjonalnie można nadpisać:
        EDNOR_DATA_ROOT=C:\\EdnorData
    """
    env = os.getenv("EDNOR_DATA_ROOT")
...
def cutting_dir() -> Path:
    # Bez polskich znaków w nazwie folderu — bezpieczniej pod Git Bash/EXE.
...
def get_path(key: str) -> str:
    """Prosty resolver ścieżek dla EDNOR."""
    ensure_data_tree()
    mapping = {
    ...
    if key not in mapping:
        raise KeyError(f"Nieznany klucz ścieżki EDNOR: {key}")
